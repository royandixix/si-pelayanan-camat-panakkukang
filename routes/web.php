...
use App\Http\Controllers\Masyarakat\LayananController;
...
Route::middleware('guest')->group(function (): void {
    Route::get(
        '/login',
        [LoginMasyarakatController::class, 'create'],
    )->name('login');

    Route::post(
        '/login',
        [LoginMasyarakatController::class, 'store'],
    )->name('login.store');

    Route::get(
        '/register',
        [RegistrasiMasyarakatController::class, 'create'],
    )->name('register');

    Route::post(
        '/register',
        [RegistrasiMasyarakatController::class, 'store'],
    )->name('register.store');
});
...
Route::middleware('auth')
    ...
    ->group(function (): void {
        ...
        Route::get(
            '/permohonan',
            [PermohonanController::class, 'index'],
        )->name('permohonan.index');

        Route::get(
            '/permohonan/buat/{layanan:slug}',
        ...
        Route::get(
            '/permohonan/{permohonan}/bukti',
            [BuktiController::class, 'show'],
        )->name('permohonan.bukti.show');

        Route::get(
            '/permohonan/{permohonan}/bukti/unduh',
            [BuktiController::class, 'download'],
        )->name('permohonan.bukti.download');

        Route::get(
            '/permohonan/{permohonan}/hasil/unduh',
            [HasilPelayananController::class, 'download'],
        )->name('permohonan.hasil.download');

        Route::get(
            '/permohonan/{permohonan}/dokumen/{dokumen}/unduh',
            [DokumenPermohonanController::class, 'download'],
        )->name('permohonan.dokumen.download');

        Route::patch(
            '/permohonan/{permohonan}/dokumen/{dokumen}/revisi',
            [RevisiDokumenController::class, 'update'],
        )->name('permohonan.dokumen.update');

        Route::post(
            '/permohonan/{permohonan}/kirim-ulang',
            [RevisiDokumenController::class, 'kirimUlang'],
        )->name('permohonan.kirim-ulang');

        Route::get(
            '/antrean',
            [AntreanController::class, 'index'],
        )->name('antrean.index');

        Route::get(
            '/antrean/{antrean}',
            [AntreanController::class, 'show'],
        )->name('antrean.show');
        ...
        Route::post(
            '/notifikasi/{notifikasi}/buka',
            [NotifikasiController::class, 'buka'],
        )->name('notifikasi.buka');

        Route::patch(
            '/notifikasi/{notifikasi}/baca',
            [NotifikasiController::class, 'baca'],
        ...
        Route::get(
            '/profil/kata-sandi',
            [ProfilController::class, 'kataSandi'],
        )->name('profil.kata-sandi');

        Route::patch(
            '/profil/kata-sandi',
            [ProfilController::class, 'updateKataSandi'],
        )->name('profil.kata-sandi.update');
    });

Route::post(
    '/logout',
    [LoginMasyarakatController::class, 'destroy'],
)
    ->middleware('auth')
    ->name('logout');